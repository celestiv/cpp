...
S21Matrix::S21Matrix(int rows, int cols)
  ...
  if (rows < 0 || cols < 0) {
    throw std::invalid_argument("s21::S21Matrix::parameterized_constructor: rows or columns less or equal to zero");
  }
  CreateMatrix();
}
...
void S21Matrix::set_rows(int new_rows) {
  if (new_rows < 0) {
    throw std::invalid_argument(
        "s21::S21Matrix::set_rows: Incorrect input, rows should be greater then 0");
  }
...
void S21Matrix::set_cols(int new_cols) {
  if (new_cols < 0) {
    throw std::invalid_argument(
        "s21::S21Matrix::set_cols: Incorrect input, cols should be greater then 0");
  }
...
double &S21Matrix::operator()(int row, int col) {
  if (row < 0 || col < 0 || row >= rows_ || col >= cols_) {
    throw std::out_of_range("s21::S21Matrix::operator(): Incorrect input, index is out of range");
  }
...
double S21Matrix::operator()(int row, int col) const {
  if (row < 0 || col < 0 || row >= rows_ || col >= cols_) {
    throw std::out_of_range("s21::S21Matrix::const operator(): Incorrect input, index is out of range");
  }
  return matrix_[row][col];
}
...
void S21Matrix::MulMatrix(const S21Matrix other) {
  if (cols_ != other.rows_) {
    throw std::invalid_argument(
        "s21::S21Matrix::MulMatrix: first matrix rows and second matrix cols don't match");
  }
...
double S21Matrix::Determinant() {
  if (rows_ != cols_) {
    throw std::invalid_argument("s21::S21Matrix::Determinant: The matrix must be square");
  }
  double result = 0.0;
  if (rows_ == 1) {
    result = matrix_[0][0];
...
S21Matrix S21Matrix::CalcComplements() {
  if (rows_ != cols_) {
    throw std::invalid_argument("s21::S21Matrix::CalcComplements: The matrix must be square");
  }
...
S21Matrix S21Matrix::InverseMatrix() {
  ...
  if (det == 0.0) {
    throw std::invalid_argument("s21::S21Matrix::InverseMatrix: Determinant must not be zero");
  }
  S21Matrix result;
  if (rows_ == 1) {
    result.set_rows(1);
...
void S21Matrix::MatrixOperation(const S21Matrix other, int coefficient) {
  if (rows_ != other.rows_ || cols_ != other.cols_) {
    throw std::out_of_range("s21::S21Matrix::MatrixOperation: Different sizes of matrices");
  }
  for (int i = 0; i < rows_; i++) {
    for (int j = 0; j < cols_; j++) {
